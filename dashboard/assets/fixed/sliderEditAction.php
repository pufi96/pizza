<?php
    ...
    $statement=$connection->prepare('UPDATE slider
                                    SET subHeading=:subHeading,mainHeading=:mainHeading,postHeading=:postHeading
    ...
    if(!empty($_FILES['uploadImgSlider'])){
        $extension =$types[$_FILES['uploadImgSlider']['type']];
        $statement=$connection->prepare('UPDATE slider
                                        SET srcImgSlider=:srcImgSlider,extImgSlider=:extImgSlider,altImgSlider=:altImgSlider
                                        WHERE slider.idSlider=:id');
        $statement->bindParam('srcImgSlider', $_POST['srcImgSlider']);
        $statement->bindParam('extImgSlider', $extension);
        ...
        $link ="../../../images/".$_POST['currentSrcImgSlider'].'.'.$_POST['currentExtImgSlider'];
        if(file_exists($link)){
            unlink($link);
        }
    ...
    $result = "Slider is successfully edited in database.";
